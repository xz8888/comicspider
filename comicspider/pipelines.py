...
class DuplicateTitlePipeline(object):
	def __init__(self):
	...
	def process_item(self, item, spider):
		posts = self.db.posts
		title = item['title']
		total = posts.find({"title": title}).count()
		if total > 0: 
			raise DropItem("Title already found %s" % title)
		else:
			return item

class DatabaseStoragePipline(object):
	...
	def process_item(self, item, spider):
		...
		if item is not None:
			theItem = dict(item)
			posts.insert(theItem)
			log.msg('Item written to MongoDB database %s/%s' % ('yuna', posts))
		else:
			log.msg('Item could not be written')
		return item
...
class CustomImagePipeline(FilesPipeline):
	...
	def file_path(self, request, response=None, info=None):
		...
		today = date.today() 
		filepath = '%s_%s_%s' % (today.year, today.month, today.day)

		return '%s/%s.jpg' % (filepath, image_guid)
